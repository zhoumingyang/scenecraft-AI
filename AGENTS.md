...
- keep studio scene geometry, temporary virtual lights, room safety lights (`roomFill`, `wallWash`, `ceilingWash`), reflector/negative-fill/strip-panel meshes, camera changes, plinth auto-fit, scene env/post edits, IBL/HDRI environment changes, and target scale/rotation session-only; transient studio entities may exist in `EditorProjectModel` while studio mode is active but must be filtered from saved project snapshots and removed on exit
...
- studio post-processing is split between temporary scene `envConfig` edits for existing tone mapping / exposure / post passes and a runtime-only studio color grading pass for contrast, saturation, temperature, tint, vignette, and detail; LUT import, LUT resources, and LUT persistence are not currently implemented
